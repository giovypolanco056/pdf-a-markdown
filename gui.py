...
No requiere dependencias extra: Tkinter viene incluido con Python.
La conversión corre en un hilo aparte para que la ventana no se congele.
"""
...
class ConverterGUI:
    ...
    def _build_ui(self) -> None:
        ...
        frm_in = ttk.LabelFrame(self.root, text="1) Archivos PDF a convertir")
        frm_in.pack(fill="both", expand=True, padx=10, pady=4)

        lst_wrap = ttk.Frame(frm_in)
        lst_wrap.pack(side="left", fill="both", expand=True, padx=8, pady=8)
        self.listbox = tk.Listbox(lst_wrap, selectmode="extended", height=8,
                                  activestyle="none")
        ...
        # --- Salida ---
        frm_out = ttk.LabelFrame(self.root, text="2) Carpeta de salida (.md)")
        ...
        ttk.Button(frm_out, text="Cambiar…", command=self.choose_output).pack(
            side="right", padx=8, pady=8)

        # --- Opciones ---
        frm_opt = ttk.LabelFrame(self.root, text="3) Opciones")
        ...
        row1 = ttk.Frame(frm_opt); row1.pack(fill="x", padx=8, pady=(6, 0))
        ...
        row2 = ttk.Frame(frm_opt); row2.pack(fill="x", padx=8, pady=(2, 8))
        ...
        ttk.Button(frm_go, text="Abrir carpeta Markdown", command=self.open_output).pack(side="right")
        ...
        self.status_var = tk.StringVar(value="Listo. Agrega PDFs y pulsa Convertir.")
        ttk.Label(self.root, textvariable=self.status_var).pack(anchor="w", padx=10, pady=(2, 4))

        # --- Registro (log) ---
        frm_log = ttk.LabelFrame(self.root, text="Resultado")
